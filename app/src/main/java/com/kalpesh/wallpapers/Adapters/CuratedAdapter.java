package com.kalpesh.wallpapers.Adapters;
...
import com.kalpesh.wallpapers.R;
...
public class CuratedAdapter extends RecyclerView.Adapter<CuratedViewHolder>{
    ...
    Context context;
    List<Photo> list;
    OnRecyclerClickedListener listener;

    public CuratedAdapter(Context context, List<Photo> list, OnRecyclerClickedListener listener) {
    ...
    public void onBindViewHolder(@NonNull CuratedViewHolder holder, int position) {
        Picasso.get().load(list.get(position).getSrc().getMedium()).into(holder.imageView_list);
